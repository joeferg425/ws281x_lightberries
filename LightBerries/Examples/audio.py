...
def plot_stuff(q):
	...
	lfArray = LightFunctions.LightPattern.ConvertPixelArrayToNumpyArray(LightFunctions.LightPattern.SolidColorArray(100, LightFunctions.PixelColors.GREEN))
...
try:
	...
	try:
		...
		stream = p.open(format=form_1, rate=sampleRate, channels=chans, input_device_index=dev_index, input=True, frames_per_buffer=bytesPerChunk)
		frames = []
		while True:
