...
class LightString(Sequence[np.int_]):
    def __init__(
        ...
        """
        # self._ledCount: int = None
        # self.pixelStrip: rpi_ws281x.PixelStrip = None
        # self._lights: np.ndarray = None
        ...
        try:
            ...
            raise
        try:
            if self._ledCount is None:
                raise Exception(
                    'Cannot create LightString object with LED count "None"',
                )
            else:
                self._lights: NDArray = np.array([Pixel() for i in range(self._ledCount)])
                LOGGER.debug(
    ...
    def __del__(self) -> None:
        ...
        """
        if self.pixelStrip is not None:
            self.off()
            try:
    ...
    def __setitem__(self, key: Union[int, slice], value: Union[Pixel, Sequence[Pixel], NDArray]) -> None:
        """
