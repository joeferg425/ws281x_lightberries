...
class MatrixFunction(ArrayTransform):
    ...
    def functionMatrixFadeOff(
        ...
        """
        try:
            ArrayTransform.Controller.virtualLEDBuffer[:, :] = ArrayTransform.Controller.virtualLEDBuffer * (
                1 - fade._fade_amount
            )
    ...
    def functionMatrixFade(
        ...
        try:
            ...
            if _fadeAmount < 0:
                ...
                _fadeAmount = 255
            for x in range(ArrayTransform.Controller.realLEDYaxisRange):
                for y in range(ArrayTransform.Controller.realLEDXaxisRange):
                    for rgbIndex in range(len(fade._color)):
                        if ArrayTransform.Controller.virtualLEDBuffer[x, y, rgbIndex] != fade._color[rgbIndex]:
                            if (
                                ArrayTransform.Controller.virtualLEDBuffer[
                                    x,
                                    y,
                                    rgbIndex,
                                ]
    ...
    def functionMatrixBounce(
        ...
        try:
            ...
            if len(ArrayTransform.Controller.virtualLEDBuffer.shape) == 2:
                ArrayTransform.Controller.virtualLEDBuffer[bounceRange] = bounce._color
            else:
                ArrayTransform.Controller.virtualLEDBuffer[bounceRange] = bounce._color
