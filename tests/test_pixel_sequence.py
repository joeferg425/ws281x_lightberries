...
def test_sequence_default_monthly() -> None:
...
def test_sequence_off_list() -> None:
    ...
    sequence = SequenceOff(led_count=led_count)
    if led_count is None:
        led_count = len(SequenceOff.get_monthly_color_sequence())
    assert sequence is not None
    assert len(sequence.array.shape) == TWO
    assert sequence.array.shape[0] == led_count
    assert sequence.array.shape[1] == THREE
...
def test_sequence_transition_len_zero() -> None:
    led_count = 0
    pixel_sequence = PixelSequence(led_count=led_count)
    sequence = SequenceTransition(led_count=led_count, wrap=True, pixel_sequence=pixel_sequence)
    assert sequence is not None
    assert len(sequence.array.shape) == TWO
    assert sequence.array.shape[0] == led_count
    assert sequence.array.shape[1] == THREE
...
def test_sequence_rainbow(led_count: int | None) -> None:
    ...
    if led_count is None:
        led_count = SequenceRainbow.get_monthly_color_sequence().led_count
    assert sequence is not None
    assert len(sequence.array.shape) == TWO
    assert sequence.array.shape[0] == led_count
    assert sequence.array.shape[1] == THREE
...
def test_sequence_random_index() -> None:
    ...
    idxs = [sequence.get_random_index() for _ in range(5)]
...
def test_sequence_default(led_count: int | None) -> None:
    sequence = SequenceDefault(led_count=led_count)
    assert sequence is not None
    assert len(sequence.array.shape) == TWO
    assert (
        sequence.array.shape[0] == led_count
        if led_count is not None
        else len(SequenceDefault.get_monthly_color_sequence())
    )
    assert sequence.array.shape[1] == THREE
    assert sequence._name == SequenceDefault.__name__  # type: ignore
