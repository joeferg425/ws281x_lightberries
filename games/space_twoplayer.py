...
def run_space_two_player(lights: MatrixController):
    ...
    for i in range(10):
        ...
        firework.rowIndex = random.randint(0, lights.realLEDYaxisRange - 1)
        firework.columnIndex = random.randint(0, lights.realLEDXaxisRange - 1)
        firework.size = 1
        firework.step = 1
    ...
    WIN_DURATION = 10
    pause = True
    PAUSE_START_DELAY = 2.0
    PAUSE_STOP_DELAY = 1.0
    pause_time = time.time() - PAUSE_START_DELAY
    ...
    while not exiting:
        ...
        if p1.dead and p2.dead:
            if time.time() - p1_dead_time > 1:
                p1.dead = False
                p1.x = random.randint(0, lights.realLEDYaxisRange - 1)
                p1.y = random.randint(0, lights.realLEDXaxisRange - 1)
        ...
        if p2.dead:
            if time.time() - p2_dead_time > RESPAWN_TIME:
                p2.dead = False
                p2.x = random.randint(0, lights.realLEDYaxisRange - 1)
                p2.y = random.randint(0, lights.realLEDXaxisRange - 1)
                p2.score = 1
                fade.color = PixelColors.OFF.array

        fade.run()
        if time.time() - p1_death_ray_time >= DEATH_RAY_DELAY:
            lights.virtualLEDBuffer[: p1.score, 0, :] = ArrayPattern.ColorTransitionArray(p1.score, READY)
        else:
            lights.virtualLEDBuffer[: p1.score, 0, :] = ArrayPattern.ColorTransitionArray(p1.score, NOT_READY)
        if time.time() - p2_death_ray_time >= DEATH_RAY_DELAY:
            lights.virtualLEDBuffer[-p2.score :, 0, :] = ArrayPattern.ColorTransitionArray(p2.score, READY)
        else:
            lights.virtualLEDBuffer[-p2.score :, 0, :] = ArrayPattern.ColorTransitionArray(p2.score, NOT_READY)
        for event in events:
            if "joy" in event.dict and "axis" in event.dict:
                ...
                if event.dict["axis"] == XboxJoystick.JOY_LEFT_X:
                    ...
                    if event.dict["joy"] == 0:
                        ...
                        ) and not (pause or fake_pause or p1.dead):
                            death_rays.append(
                                sprite(
                                    name="death ray",
                                    x=p1.x,
                        ...
                        ) and not (pause or fake_pause or p2.dead):
                            death_rays.append(
                                sprite(
                                    name="death ray",
                                    x=p2.x,
            ...
            if "joy" in event.dict and "button" in event.dict:
                if event.dict["button"] == XboxButton.START:
                    if pause and time.time() - pause_time > PAUSE_STOP_DELAY:
                        pause_time = time.time()
                        pause = not pause
                    elif not pause and time.time() - pause_time > PAUSE_START_DELAY:
                        pause_time = time.time()
                        pause = not pause
        ...
        if not p1.dead and not pause:
            p1.go()
            lights.virtualLEDBuffer[p1.xs, p1.ys] = Pixel(p1.color).array
        if not p2.dead and not pause:
            p2.go()
            lights.virtualLEDBuffer[p2.xs, p2.ys] = Pixel(p2.color).array
