...
class LightGame:
    ...
    def __init__(self, lights: MatrixController) -> None:
                ...
                for j in range(32):
                    self.rs.append(
                        pygame.draw.rect(
                            ...
                            (
                                (
                                    (i * LightGame.SIMULATED_SIZE) + 1,
                                    (j * LightGame.SIMULATED_SIZE) + 1,
                                ),
                                (
                                    LightGame.SIMULATED_SIZE - 2,
                                    LightGame.SIMULATED_SIZE - 2,
                                ),
                            ),
                        )
        ...
        self.fade = ArrayFunction(
            lights,
            MatrixFunction.functionMatrixFadeOff,
    ...
    def update_game(self):
        if self.first_render or not (self.pause or self.exiting):
            ...
            if not self.win:
                ...
                # check_for_collisions(self.lights)
